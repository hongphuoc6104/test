...
def main():
    ...
    sim_threshold = search_cfg.get("sim_threshold", 0.5)
    margin_threshold = search_cfg.get("margin_threshold", 0.05)
    top_k = max(2, search_cfg.get("knn", 5))
    ...
    valid_matches = [m for m in matches if m.get("distance", 0.0) >= sim_threshold]
